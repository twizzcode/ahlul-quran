...
export default async function DashboardPage() {
  ...
  const stats = [
    {
      ...
      helper: `${monthlyDonationCount} donasi sukses`,
      href: ADMIN_ROUTE_PATHS.donasi,
    },
    {
      label: "Campaign Aktif",
      value: activeCampaignCount.toString(),
      helper: "Program yang masih berjalan",
      href: ADMIN_ROUTE_PATHS.kampanye,
    },
    {
      label: "Konten & Media",
      value: `${articleCount} / ${galleryCount}`,
      helper: "Artikel dan galeri tersimpan",
  ...
  return (
    <div className="flex flex-1 flex-col gap-6">
      <section className="rounded-[28px] border border-emerald-100 bg-white p-6 shadow-sm md:p-7">
            ...
            <div className="mt-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="rounded-2xl bg-white px-4 py-3">
                  ...
                  <p className="mt-1 text-xl font-semibold text-slate-950">{activeCampaignCount}</p>
                </div>
                <div className="rounded-2xl bg-white px-4 py-3">
                  <p className="text-slate-500">Admin</p>
                  <p className="mt-1 text-xl font-semibold text-slate-950">{userCount}</p>
                </div>
              </div>
              <div className="flex items-start gap-3 rounded-2xl bg-white px-4 py-3">
                ...
                <p className="text-sm leading-6 text-slate-600">
                  Data dashboard mengikuti schema dan session admin yang sedang aktif.
                </p>
              </div>
            </div>
          </div>
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-lg font-semibold text-slate-950">Aktivitas Terbaru</h2>
              <p className="text-sm text-slate-500">Pantau konten dan donasi yang paling baru masuk.</p>
            </div>
            <Link href={ADMIN_ROUTE_PATHS.artikel} className="text-sm font-semibold text-emerald-800 hover:underline">
              Lihat semua
            </Link>
          </div>
          ...
          <div className="mt-5 grid gap-6 lg:grid-cols-2">
            <div>
              <h3 className="text-sm font-semibold text-slate-900">Artikel Terbaru</h3>
              <div className="mt-3 space-y-3">
              {recentArticles.length === 0 ? (
                <div className="rounded-2xl border border-dashed p-4 text-sm text-slate-500">Belum ada artikel.</div>
              ) : (
                recentArticles.map((item) => (
                  <Link key={item.id} href={ADMIN_ROUTE_PATHS.artikel} className="block rounded-2xl border border-slate-200 p-4 transition hover:border-emerald-300 hover:bg-emerald-50/40">
            ...
            <div>
              <h3 className="text-sm font-semibold text-slate-900">Donasi Sukses Terbaru</h3>
              <div className="mt-3 space-y-3">
        ...
        <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-slate-500">Akses cepat ke area pengelolaan inti.</p>
          <div className="mt-5 space-y-3">
            {moduleCards.slice(0, 5).map((item) => {
              ...
              return (
                <Link
                  key={item.url}
                  href={item.url}
                  className="flex items-start gap-3 rounded-2xl border border-slate-200 px-4 py-3 transition hover:border-emerald-300 hover:bg-emerald-50/50"
                >
