...
import { HomeContentTabs } from "@/components/home/home-content-tabs";
...
export default async function HomePage() {
  ...
  const [campaigns, selectedHomepageCampaignRaw, latestArticles] = await Promise.all([
    ...
    profile.homepageFeatureCampaignId
      ? dbQuery.donationCampaign.findFirst({
          where: {
            id: profile.homepageFeatureCampaignId,
            isActive: true,
          },
          include: {
            donations: {
  ...
  const fallbackHomepageCampaign = donationCampaigns[0] ?? null;

  const homepageCampaign = (() => {
    if (selectedHomepageCampaignRaw) {
      const collectedAmount = selectedHomepageCampaignRaw.donations.reduce(
        (sum, d) => sum + d.amount,
        0
      );
      const progress =
        selectedHomepageCampaignRaw.targetAmount > 0
          ? Math.min(
              100,
              Math.round((collectedAmount / selectedHomepageCampaignRaw.targetAmount) * 100)
            )
          : 0;

      return {
        id: selectedHomepageCampaignRaw.id,
        slug: selectedHomepageCampaignRaw.slug,
        title: selectedHomepageCampaignRaw.title,
        description: selectedHomepageCampaignRaw.description,
        coverImage: selectedHomepageCampaignRaw.coverImage,
        targetAmount: selectedHomepageCampaignRaw.targetAmount,
        collectedAmount,
        progress,
        endDate: selectedHomepageCampaignRaw.endDate
          ? selectedHomepageCampaignRaw.endDate.toISOString()
          : null,
        supporters: Array.from(
          selectedHomepageCampaignRaw.donations.reduce((acc, donation) => {
            const name = donation.isAnonymous ? "Hamba Allah" : donation.donorName;
            const current = acc.get(name) ?? 0;
            acc.set(name, current + donation.amount);
            return acc;
    ...
    return fallbackHomepageCampaign;
  })();

  const heroTitle =
    profile.heroTitle.trim() || `${profile.name} #MasjidPusatSolusi`;
  const heroDescription = profile.heroSubtitle.trim() || profile.description;
  const featureBadge =
    profile.homepageFeatureBadge.trim() || profile.foundationName || profile.name;
  const featureTitle =
    profile.homepageFeatureTitle.trim() || heroTitle;
  const featureDescription =
    profile.homepageFeatureDescription.trim() || heroDescription;
  const isDonationFeatureEnabled =
    profile.homepageFeatureDonationEnabled && homepageCampaign !== null;
  const featurePrimaryButtonText = profile.homepageFeaturePrimaryButtonText.trim()
    || `Dukung ${profile.movementName}`;
  const featureProfileButtonText = profile.homepageFeatureProfileButtonText.trim()
    || "Lihat Profil Markas";

  const heroImage =
    profile.bannerUrl || homepageCampaign?.coverImage || "/Gambar-masjid.png";

  const ctaFallbackImages = [
    donationCampaigns[0]?.coverImage || "/Gambar-masjid.png",
    homepageCampaign?.coverImage || "/Gambar-masjid.png",
    displayedArticles[0]?.coverImage || "/Gambar-masjid.png",
    donationCampaigns[1]?.coverImage || "/Gambar-masjid.png",
    displayedArticles[1]?.coverImage || "/Gambar-masjid.png",
    donationCampaigns[2]?.coverImage || "/Gambar-masjid.png",
  ];

  const ctaMasonryItems = (profile.homepageCtaCards.length > 0
    ? profile.homepageCtaCards
    : [
    ...
    title: item.title,
    image: item.imageUrl || ctaFallbackImages[index] || "/Gambar-masjid.png",
    className: "h-44",
  }));

  const testimonials: Testimonial[] = profile.homepageTestimonials.map((item) => ({
    quote: item.quote,
    name: item.name,
    role: item.role,
    imgSrc: item.imageUrl || "/logo.webp",
      ...
      <section className="relative mt-[var(--home-nav-height)] w-full min-h-[calc(85vh-var(--home-nav-height))] overflow-hidden">
        ...
        <div className="relative z-10 mx-auto flex h-full min-h-[calc(85vh-var(--home-nav-height))] w-full max-w-7xl items-center px-6">
          <div className="max-w-2xl">
            ...
            {isDonationFeatureEnabled && homepageCampaign ? (
              <div className="mt-6 max-w-xl space-y-2">
                <div className="flex items-center justify-between text-sm text-white">
                  <span className="font-semibold">
                    {formatCurrency(homepageCampaign.collectedAmount)}
                  </span>
                  <span className="text-white/75">
                    dari {formatCurrency(homepageCampaign.targetAmount)}
                  </span>
                </div>
                <div className="h-2 overflow-hidden rounded-full bg-white/20">
                  <div
                    className="h-full rounded-full bg-white transition-all"
                    style={{ width: `${homepageCampaign.progress}%` }}
                  />
                </div>
                <div className="flex items-center justify-between text-xs text-white/75">
                  <span>{homepageCampaign.progress}% terkumpul</span>
                  <span>
                    {homepageCampaign.endDate
                      ? `Sampai ${formatDate(homepageCampaign.endDate)}`
                      : "Tanpa batas waktu"}
                  </span>
                </div>
              </div>
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              ...
              >
                <Link
                  href={
                    isDonationFeatureEnabled && homepageCampaign?.slug
                      ? `/donasi/${homepageCampaign.slug}`
                      : "/profil"
                  }
                >
                  {isDonationFeatureEnabled ? <Heart className="mr-2 h-4 w-4" /> : null}
                  {isDonationFeatureEnabled
                    ? featurePrimaryButtonText
                    : featureProfileButtonText}
                </Link>
              </Button>
              {isDonationFeatureEnabled ? (
                <Button
                  ...
                  asChild
                >
                  <Link href="/profil">{featureProfileButtonText}</Link>
                </Button>
              ) : null}
            </div>
          </div>
      ...
      <section className="pb-28 pt-24 sm:pb-32 sm:pt-36 lg:pt-40 xl:pt-44">
        <div className="mx-auto w-full max-w-7xl px-6">
          <div className="mb-8 max-w-2xl">
            ...
            <h2 className="mt-2 text-3xl font-bold text-emerald-950">
              Suara Jamaah dan Donatur
            </h2>
            <p className="mt-2 text-emerald-900/70">
              Beberapa kesan awal dari jamaah, relawan, dan donatur yang
              terhubung dengan gerakan {profile.movementName}.
            </p>
          </div>
        </div>
          ...
          <div className="max-w-xl">
            <h2 className="text-3xl font-bold tracking-tight text-balance text-emerald-950 md:text-4xl">
              {profile.homepageCtaTitle.trim() ||
                ...
                `Bangun ${profile.name} bersama gerakan yang terarah`}
            </h2>
            <p className="mt-6 max-w-lg text-base text-emerald-900/70">
              {profile.homepageCtaDescription.trim() ||
                profile.heroSubtitle.trim() ||
                `Setiap kontribusi membantu ${profile.name} bertumbuh menjadi pusat ibadah, dakwah, pendidikan, dan pelayanan umat. Dukungan hari ini menjadi fondasi program yang hidup untuk jangka panjang.`}
            </p>
            ...
            >
              <Link href="/donasi">
                <span>{profile.homepageCtaButtonText.trim() || "Dukung Pembangunan"}</span>
