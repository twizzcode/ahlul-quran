...
type DashboardCampaignDetailProps = {
  ...
  articleOptions: DashboardCampaignArticleOption[];
          ...
          <section className="rounded-[24px] border bg-card p-5">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="min-w-0">
                ...
                <h2 className="mt-2 line-clamp-2 text-2xl font-bold tracking-tight text-slate-900">
                  {campaign.title}
                </h2>
                <p className="mt-2 text-sm leading-6 text-muted-foreground">
                  Perbarui informasi kampanye, visual cover, target donasi, dan berita update
                  yang ditautkan.
                </p>
              </div>
                ...
                {articleOptions.length === 0 ? (
                  ...
                  <div className="max-h-80 space-y-2 overflow-y-auto rounded-xl border p-3">
                    {articleOptions.map((article) => {
                      const isChecked = campaign.linkedArticles.some((item) => item.id === article.id);
